...
export default {
    ...
    resolveSymbol: async (symbolName, onSymbolResolvedCallback, onResolveErrorCallback) => {
        ...
        const symbols = await getAllSymbols();
        console.log('all symbols', symbols.filter(item => item.type !== "crypto"));
    ...
    getBars: async (
        ...
        try {
            const data = await makeApiRequestForChart(
                `${process.env.API_BASE_URL}trade/${parsedSymbol.fromSymbol}-${parsedSymbol.toSymbol}/chart^?${url}&${query}`,
            );
            ...
            data?.results.forEach((bar, i, array) => {
    ...
    subscribeBars: (
        ...
        const lastBarinCookie = Cookies.get('lastBarSpot') ? Cookies.get('lastBarSpot') : null;
        subscribeOnStream(
            symbolInfo,
            resolution,
            onRealtimeCallback,
            subscribeUID,
            onResetCacheNeededCallback,
            JSON.parse(lastBarinCookie && lastBarinCookie),
        );
    },
